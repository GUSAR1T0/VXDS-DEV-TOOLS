...
namespace VXDesign.Store.DevTools.Common.Containers.Properties
{
    public abstract class CamundaWorkersProperties : IPropertiesMarker
    {
        ...
        public int CountOfRetriesWhenFetchIsUnsuccessful { get; set; } = 10;

        [PropertyField]
        public int RetryAfterFetchTimeout { get; set; } = (int) TimeSpan.FromSeconds(10).TotalMilliseconds;
        ...
        [PropertyField]
        public int LockDuration { get; set; } = (int) TimeSpan.FromSeconds(10).TotalMilliseconds; // (int) TimeSpan.FromMinutes(5).TotalMilliseconds;
